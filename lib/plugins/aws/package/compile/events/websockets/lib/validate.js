      ...
      functionObject.events.forEach(event => {
        ...
        if (_.has(event, 'websocket')) {
          ...
          if (_.isObject(event.websocket)) {
            ...
            // route request model
            if (event.websocket.request && event.websocket.request.schema) {
              if (
                !_.isObject(event.websocket.request.schema) ||
                Object.keys(event.websocket.request.schema).length !== 1
              ) {
                throw new this.serverless.classes.Error('You must specify a single content-type');
              }
              const contentType = Object.keys(event.websocket.request.schema)[0];
              if (contentType !== 'application/json') {
                throw new this.serverless.classes.Error('content-type must be "application/json"');
              }
              if (!_.isObject(event.websocket.request.schema[contentType])) {
                throw new this.serverless.classes.Error(
                  'You must specify a JSON schema object to validate against'
                );
              }
              websocketObj.request = event.websocket.request;
            }
